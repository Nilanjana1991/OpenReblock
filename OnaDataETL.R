...
server <- function(input, output, session) 
  ...
  output$leafmap <- renderLeaflet({
    leaflet() %>%
      ...
      addPolygons(data = sle_garbagedump, label = ~settlement, fillColor= ~pal_garbagedump(as.integer(garbagedum)), fillOpacity = 0.7, weight = 2, group = "Hazard: Physical") %>%
      addPolygons(data = sle_industrial, label = ~settlement, fillColor= ~pal_industrial(as.integer(industrial)), fillOpacity = 0.7, weight = 2, group = "Hazard: Physical") %>%
